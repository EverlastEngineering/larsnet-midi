...
# 2026-06-22: The /api/reclassify endpoint was removed. The Tune
# panel now re-classifies events entirely client-side — every
# hihat event carries `decay_slope_db` in the sidecar, and the
# open/closed threshold is just a value compare. Save & Reconvert
# still calls /api/rebuild-midi to persist the new threshold to
# midiconfig.yaml and stamp the result into the sidecar.
